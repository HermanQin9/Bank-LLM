...
public class EnhancedFraudDetectionService {
    ...
    public EnhancedFraudDetectionService(
            ...
            String pythonApiUrl) {
        this(transactionRepository, pythonApiUrl, null);
    }

    public EnhancedFraudDetectionService(
            TransactionRepository transactionRepository,
            String pythonApiUrl,
            TransactionKafkaPublisher kafkaPublisher) {
        ...
        if (kafkaPublisher != null && kafkaPublisher.isEnabled()) {
            logger.info("Kafka streaming enabled - alerts will be published to downstream services");
        }
    }
    ...
    public FraudAlert analyzeTransaction(Transaction transaction) {
            ...
            try {
                ...
                if (!llmAnalysis.containsKey("error")) {
                    ...
                    if (llmAnalysis.containsKey("supporting_documents")) {
                        ...
                        List<Map<String, Object>> docs = (List<Map<String, Object>>) llmAnalysis
                                .get("supporting_documents");
                        documentEvidence = docs;
        ...
        if (kafkaPublisher != null && kafkaPublisher.isEnabled()) {
            kafkaPublisher.publishAlert(transaction, alert, ruleBasedScore, finalScore);
        }
    ...
    public List<Map<String, Object>> searchCustomerDocuments(String customerId, String query) {
        ...
        try {
            ...
            if (!searchResult.containsKey("error")) {
                @SuppressWarnings("unchecked")
                List<Map<String, Object>> documents = (List<Map<String, Object>>) searchResult.get("documents");
    ...
    private FraudAlert.RiskLevel determineRiskLevel(double score) {
        if (score >= 0.8)
            return FraudAlert.RiskLevel.CRITICAL;
        if (score >= 0.6)
            return FraudAlert.RiskLevel.HIGH;
        if (score >= 0.4)
            return FraudAlert.RiskLevel.MEDIUM;
