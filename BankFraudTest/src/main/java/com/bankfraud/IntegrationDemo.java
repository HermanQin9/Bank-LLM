...
import com.bankfraud.streaming.TransactionKafkaPublisher;
...
public class IntegrationDemo {
    ...
    public static void main(String[] args) {
        ...
        String kafkaTopic = System.getenv().getOrDefault("KAFKA_ALERT_TOPIC", "fraud.alerts");
        String kafkaClientId = System.getenv().getOrDefault("KAFKA_CLIENT_ID", "fraud-alert-demo");
        TransactionKafkaPublisher kafkaPublisher = null;
        if (kafkaBootstrap != null && !kafkaBootstrap.isBlank()) {
            kafkaPublisher = new TransactionKafkaPublisher(kafkaBootstrap, kafkaTopic, kafkaClientId);
        }

        EnhancedFraudDetectionService fraudService = new EnhancedFraudDetectionService(transactionRepo, pythonApiUrl,
                kafkaPublisher);
        ...
        logger.info("  Kafka streaming: {}",
                kafkaPublisher != null && kafkaPublisher.isEnabled() ? "ENABLED" : "DISABLED");
        ...
        logger.info("  - Kafka: Streaming bridge between Java rules and Python LLM automations");
